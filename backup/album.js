...
function renderMovieData(data) {
    ...
    // 7. Episodes Logic & Main Play Button
    if (episodes && episodes.length > 0) {
        currentServerData = episodes[0].server_data;
        const totalEpisodes = currentServerData.length;
        const chunkSize = 24;

        // CẬP NHẬT: Button Chiếu phát chuyển hướng sang play.html?{slug}
        const btnPlayMain = document.getElementById('btnPlayMain');
        btnPlayMain.onclick = function () {
            if (currentMovieInfo && currentMovieInfo.slug) {
                window.location.href = `play.html?${currentMovieInfo.slug}`;
            }
        }
...
function renderEpisodeGrid(startIndex, limit) {
    ...
    pageData.forEach(ep => {
        // CẬP NHẬT: Tạo URL theo định dạng play.html?{slug-phim}-{slug-tap}
        // ep.slug thường có dạng "tap-1", "tap-2"
        const destUrl = `play.html?${currentMovieInfo.slug}-${ep.slug}`;

        episodeHtml += `
        <div class="card" onclick="window.location.href='${destUrl}'">
