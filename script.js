    ...
    // --- FETCH ROMANCE MOVIES (NEW API WITH LIMIT) ---
    async function fetchRomanceMovies() {
        ...
        try {
            ...
            // Cấu trúc API: data.data.items, và data.data.APP_DOMAIN_CDN_IMAGE
    ...
    // --- FETCH TOP TRENDING (MUTED COLORS) ---
    const mutedColors = [
        ...
        try {
            ...
            if (items.length > 0) {
                ...
                topItems.forEach((movie, index) => {
                    ...
                    const card = document.createElement('div');
                    card.className = 'movie-card movie-card--trending'; // Class riêng cho Top Trending
    ...
    initSlider();
    fetchNewMovies();
    fetchRomanceMovies(); // Gọi hàm mới
    fetchTopTrending();